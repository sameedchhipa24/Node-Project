...
async function Currencyconverter() {
  let CurrencyConverter:
    {
      BaseCountry: "Pakistani Rupee(PKR)" | "United States Dollar(USD)" | "Saudi Arabia(RIYAL)" | "EURO(€)"
      Amount: number
      ExchangeCurrency: "Pakistani Rupee(PKR)" | "United States Dollar(USD)" | "Saudi Arabia(RIYAL)" | "EURO(€)",
    }
